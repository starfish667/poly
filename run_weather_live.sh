...
PRICE_WEBSOCKET="${POLYBOT_PRICE_WEBSOCKET:-1}"
PRICE_WEBSOCKET_MAX_AGE="${POLYBOT_PRICE_WEBSOCKET_MAX_AGE:-10}"
LOG_FILE="logs/weather-live-$(date -u +%Y%m%d-%H%M%S).log"
PRICE_ARGS=()
if [[ "$PRICE_WEBSOCKET" != "0" ]]; then
  PRICE_ARGS+=(--price-websocket --price-websocket-max-age "$PRICE_WEBSOCKET_MAX_AGE")
fi
